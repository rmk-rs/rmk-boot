...
(=MEMORY=) for the linker and the standard embassy-boot partition symbols
(=__bootloader_*=) that both RMK and the bootloader read at runtime via
=init_flash_from_linkerscript()= and =from_linkerfile_blocking()=.
...
The =__bootloader_active_*=, =__bootloader_state_*= and =__bootloader_dfu_*=
symbols are the same ones embassy-boot's =from_linkerfile_blocking()= reads;
only =__bootloader_storage_*= is an RMK extension. If you build a custom
embassy-boot bootloader, define these same symbols in your own linker script —
RMK will pick them up automatically.
