...
via =dfu_flash_layout()=. No manual address calculation, no
...
=build.rs= writes the linker script natively, both under the generic name
=rmk-memory.x= and under the variant-specific name used by RMK on every
=build.rs= run for a supported platform feature (e.g. =rp2040-2mb= also emits
=rmk-rp2040-2mb-memory.x=). No copy step is required.

Each =cargo make uf2-*= target builds the bootloader and produces a matching
linker script:
...
cargo make uf2-nrf52833    # → rmk-boot-nrf52833.uf2 + rmk-nrf52833-memory.x
#+END_SRC
...
reads at runtime via =dfu_flash_layout()=.
...
2. Rename the =.x= file to =memory.x= and place it next to your =Cargo.toml=.
...
RMK's =dfu_flash_layout()= reads these linker symbols from
...
| nRF52840 | P0_25  | P0_23  | P0_24  | P0_22  |
...
#+BEGIN_SRC rust
let spi = Spim::new(p.TWISPI0, ExtFlashIrqs, p.P0_25, p.P0_24, p.P0_23, spi_cfg);
...
=dfu_flash_layout()= reads at runtime (all flash-relative offsets):
