...
///
/// `ERASE_UNIT` is the erase granularity exposed to the boot flow: 64K block
/// erase, matching the 64K-multiple ACTIVE partition sizing on both RP2040
/// and nRF52 with `dfu_ext` (4K when not using `dfu_ext`).
pub struct W25qNorFlash<BUS: SpiBus, CS: OutputPin, const ERASE_UNIT: usize> {
    bus: BUS,
    cs: CS,
    flash_size: u32,
}
...
impl<BUS: SpiBus, CS: OutputPin, const ERASE_UNIT: usize> ErrorType
    for W25qNorFlash<BUS, CS, ERASE_UNIT>
{
    type Error = W25qError<BUS::Error>;
}

impl<BUS: SpiBus, CS: OutputPin, const ERASE_UNIT: usize> ReadNorFlash
    for W25qNorFlash<BUS, CS, ERASE_UNIT>
{
...
impl<BUS: SpiBus, CS: OutputPin, const ERASE_UNIT: usize> NorFlash
    for W25qNorFlash<BUS, CS, ERASE_UNIT>
{
    const WRITE_SIZE: usize = 1;
    const ERASE_SIZE: usize = ERASE_UNIT;
...
impl<BUS: SpiBus, CS: OutputPin, const ERASE_UNIT: usize> MultiwriteNorFlash
    for W25qNorFlash<BUS, CS, ERASE_UNIT>
{
}
